...
class Grid
{
    ...
    public function render(): string
    {
        ...
        return view('grid_view::grid', [
            ...
            'useInlineEdit' => $this->getUseInlineEdit(),
    ...
    protected function applyColumnsConfig(): void
    {
        foreach ($this->columnFields as $key => $config) {
            ...
            $inlineEditSubConfig = $this->getUseInlineEdit() ? [] : ['inlineEdit' => false, 'inlineEditBox' => ""];

            if (is_string($config)) {
                $config = array_merge(['attribute' => $config], $filterSubConfig, $inlineEditSubConfig);
            ...
            if (is_array($config)) {
                ...
                if (isset($config['inlineEdit']) && $config['inlineEdit'] instanceof Closure) {
                    $this->fillColumnsObjects(new CallbackColumn($config));
                    continue;
                }

                if (isset($config['inlineEditBox']) && $config['inlineEditBox'] instanceof Closure) {
                    $this->fillColumnsObjects(new CallbackColumn($config));
                    continue;
                }
                $this->fillColumnsObjects(new DefaultColumn($config));
    ...
    protected function getUseInlineEdit()
    {
        return $this->useInlineEdit;
    }
}